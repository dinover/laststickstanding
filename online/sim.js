/* Host-authoritative simulation of Last Stick Standing. Lives here, in online/, because the
   authority is this server: sim-host.js reads this file straight off disk into a per-room vm
   context, and server.js also serves it to the browser at /sim.js so the client can render
   guest frames (and run Práctica Libre / Historia / Local entirely on its own) with the exact
   same code. It used to live in desktop/ back when that build was a PeerJS host that simulated
   locally; that build is now just an Electron window pointed at this server.

   Transport-agnostic on purpose: no ac.* / FreeConsole calls anywhere. Deliberately NOT shared
   with screen.html — that one keeps its own older copy of the gameplay, untouched.
